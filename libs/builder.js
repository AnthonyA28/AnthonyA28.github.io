...
if ( GAME_JSON == undefined ) {
    ...
    PXMPS   = JSON.parse(GAME_JSON);
    document.getElementById("json").value = JSON.stringify(PXMPS);
}
...
docElem("copy").onclick = function () {
    ...
    // G_stamp = clone_slice({x:G_rect.x1, y:G_rect.y1 }, {x:G_rect.x2, y: G_rect.y2}, curMap());
    docElem("json").value = JSON.stringify(G_stamp);    
}
...
docElem("export_GAMEINFO").onclick = function () {
    let myJSON = JSON.stringify(PXMPS);
    docElem("json").value = myJSON;
}
...
docElem("F0").onclick = function () {draw();}
docElem("F1").onclick = function () {draw();}
docElem("F2").onclick = function () {draw();}
docElem("F3").onclick = function () {draw();}
docElem("F4").onclick = function () {draw();}
docElem("F5").onclick = function () {draw();}
docElem("F6").onclick = function () {draw();}
docElem("F7").onclick = function () {draw();}
...
document.getElementById("import_GAMEINFO").onclick = function () {

    let text = document.getElementById("json").value;
    console.log("Importing json")
    PXMPS = JSON.parse(text);
    draw();
}
...
/* FIll the pallete with the colors in map 0 */
// let event = new MouseEvent("click");
// docElem("add_map_colors_to_pallete").dispatchEvent(event);
